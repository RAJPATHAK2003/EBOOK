...
export const Login = () => {
  ...
  async function handleSubmit(event){
   ...
   console.log(userDetail.password);
    fetch(`http://localhost:8000/users?email=${userDetail.email}`).then((res)=>{
      console.log(res);
        return res.json();
    }).then((resp)=>{
       console.log(resp);
      if(Object.keys(resp).length===0){
        ...
        if(resp.password===userDetail.password){
          toast.error('login succesfully');
      navigate("/products");
        }else{
